...
public class WaveBallView extends View {
	...
	public WaveBallView(Context context, AttributeSet attrs) {
		...
		backgroundColor = typedArray.getColor(
				R.styleable.CustomView_background_color,
				DEFUALT_BACKGROUND_COLOR);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (modeWidth == MeasureSpec.EXACTLY) {
			...
			width = Math.round((radius + STROKE_WIDTH) * 2) + getPaddingLeft()
					+ getPaddingRight();
		...
		if (modeHeight == MeasureSpec.EXACTLY) {
			...
			height = Math.round((radius + STROKE_WIDTH) * 2)
	...
	protected void onDraw(Canvas canvas) {
		...
		// {
		// int m = i + initPos;
		// tempCanvas.drawLine( i, height, i, (int)( height * ( 1.0f - percent )
		// + WAVE_HEIGHT ) + Math.round( Math.sin( Math.PI * (double)m / width )
		// * WAVE_HEIGHT ) , mWavePaint );
		// }
		...
		canvas.drawText(content, centerX - rect.width() / 2,
				centerY + rect.height() / 2, mContentPaint);
	}
